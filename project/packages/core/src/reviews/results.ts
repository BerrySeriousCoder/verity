...
export function reviewSummary(findings: ReviewFinding[]): string {
  ...
  return `Finished processing ${findings.length} directional checks. ${different} checks have verified differences; ${unresolved} remain unresolved. Reciprocal checks may describe the same discrepancy.`;
}

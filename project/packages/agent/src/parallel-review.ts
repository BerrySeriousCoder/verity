...
export async function parallelReview(input: {
  ...
  await mapConcurrent(packets, concurrency, async (packet, packetIndex) => {
    signal.throwIfAborted();
    ...
    if (needs.length) {
      for (const item of needs) {
        ...
        for (const request of item.requests) {
          ...
          bundle.evidence = [
            ...new Map(
              [...bundle.evidence, ...extra].map((block) => [block.id, block]),
            ).values(),
          ];
          await reviews.emit(
            job,
      ...
      proposed = {
        items: proposed.items.map(
          (item) => repaired.items.find((next) => next.id === item.id) ?? item,
        ),
      };
    }
    const prepared = [];
    for (const item of proposed.items) {
      ...
      const structural =
        ...
        (!['aligned', 'different'].includes(item.decision.status) || bothSides);
      const provisional: ReviewFinding = {
        ...
        evidenceIds: citationsValid ? item.decision.evidenceIds : bundle.ownIds,
    ...
    for (const entry of prepared) {
      ...
      const supported = entry.structuralChecksPassed && verification.supported;

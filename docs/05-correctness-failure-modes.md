...
Correctness has separate dimensions: faithful extraction, relevant and sufficient retrieval, correct interpretation, applicable evidence, correct synthesis and calculations, faithful final output, and authorized execution. A citation can accurately quote a source whose contents are false; grounding does not establish external truth.
...
| F34 | Source or policy changes after verification | Snapshot dependencies and invalidate affected results/approvals | Replace a source before dispatch |
...
| F36 | Approval is bypassed, reused for changed content, or granted by the agent | Server-side exact-action approval checks at dispatch | Edit destination or payload after approval |
| F37 | Retry creates duplicate external effects or competing workers commit conflicting state | Durable attempts, concurrency controls, idempotency, unknown-outcome reconciliation | Crash after external success but before local receipt |

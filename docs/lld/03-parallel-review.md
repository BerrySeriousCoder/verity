...
## 2026-09-23 — Batching version 4 reliability fixes

New runs use the evidence-context and result-state rules in [ADR 0011](../decisions/0011-evidence-context-and-result-semantics.md). Small-document context is loaded once, scoped to the confirmed relationship, and reused across checks. Larger documents retain targeted retrieval with neighboring blocks and referenced-total lookup. The independent verifier sees cited evidence plus additional original context; a deterministic dependency guard can veto unsupported alignment even when both models approve it.

`verified` now means a resolved aligned/different comparison. A verifier agreeing that a question is unresolved cannot mark it verified. Persisted exclusions expose what the agreed scope omitted. Shared comparisons retain all original directional rows, citations, and checkpoints; the questionnaire can group their presentation without deleting them. Existing runs retain their execution layout and saved findings. Start a new run to use version 4.
